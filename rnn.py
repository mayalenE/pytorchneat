...
class RecurrentNetwork(nn.Module):
    ...
    def __init__(self, input_neuron_ids, hidden_neuron_ids, output_neuron_ids, biases, responses, activations, connections, device='cpu'):
        ...
        input_to_hidden_mask = torch.zeros((self.n_inputs, self.n_hidden), dtype=torch.bool)
        hidden_to_hidden = torch.zeros((self.n_hidden, self.n_hidden))
        hidden_to_hidden_mask = torch.zeros((self.n_hidden, self.n_hidden), dtype=torch.bool)
        output_to_hidden = torch.zeros((self.n_outputs, self.n_hidden))
        output_to_hidden_mask = torch.zeros((self.n_outputs, self.n_hidden), dtype=torch.bool)
        input_to_output = torch.zeros((self.n_inputs, self.n_outputs))
        input_to_output_mask = torch.zeros((self.n_inputs, self.n_outputs), dtype=torch.bool)
        hidden_to_output = torch.zeros((self.n_hidden, self.n_outputs))
        hidden_to_output_mask = torch.zeros((self.n_hidden, self.n_outputs), dtype=torch.bool)
        output_to_output = torch.zeros((self.n_outputs, self.n_outputs))
        output_to_output_mask = torch.zeros((self.n_outputs, self.n_outputs), dtype=torch.bool)
        for k, v in connections.items():
            if k[1] in hidden_neuron_ids:
                if k[0] in input_neuron_ids:
                    input_to_hidden[self.input_key_to_idx[k[0]], self.hidden_key_to_idx[k[1]]] = v
                    input_to_hidden_mask[self.input_key_to_idx[k[0]], self.hidden_key_to_idx[k[1]]] = True
                elif k[0] in hidden_neuron_ids:
                    hidden_to_hidden[self.hidden_key_to_idx[k[0]], self.hidden_key_to_idx[k[1]]] = v
                    hidden_to_hidden_mask[self.hidden_key_to_idx[k[0]], self.hidden_key_to_idx[k[1]]] = True
                elif k[0] in output_neuron_ids:
                    output_to_hidden[self.output_key_to_idx[k[0]], self.hidden_key_to_idx[k[1]]] = v
                    output_to_hidden_mask[self.output_key_to_idx[k[0]], self.hidden_key_to_idx[k[1]]] = True
            elif k[1] in output_neuron_ids:
                if k[0] in input_neuron_ids:
                    input_to_output[self.input_key_to_idx[k[0]], self.output_key_to_idx[k[1]]] = v
                    input_to_output_mask[self.input_key_to_idx[k[0]], self.output_key_to_idx[k[1]]] = True
                elif k[0] in hidden_neuron_ids:
                    hidden_to_output[self.hidden_key_to_idx[k[0]], self.output_key_to_idx[k[1]]] = v
                    hidden_to_output_mask[self.hidden_key_to_idx[k[0]], self.output_key_to_idx[k[1]]] = True
                elif k[0] in output_neuron_ids:
                    output_to_output[self.output_key_to_idx[k[0]], self.output_key_to_idx[k[1]]] = v
                    output_to_output_mask[self.output_key_to_idx[k[0]], self.output_key_to_idx[k[1]]] = True
        ...
        hidden_mask = (self.input_to_hidden_mask.sum(0)+self.hidden_to_hidden_mask.sum(0)+self.output_to_hidden_mask.sum(0)) > 0
        self.register_buffer('hidden_mask', hidden_mask)
        output_mask = (self.input_to_output_mask.sum(0) + self.hidden_to_output_mask.sum(0) + self.output_to_output_mask.sum(0)) > 0
    ...
    def forward(self, inputs):
        ...
        after_pass_hidden_activs = inputs.mm(self.input_to_hidden) + \
                                 self.hidden_activs.mm(self.hidden_to_hidden) + \
                                 self.output_activs.mm(self.output_to_hidden)  # batch_size, n_hidden

        after_pass_output_activs = inputs.mm(self.input_to_output) + \
                                 self.hidden_activs.mm(self.hidden_to_output) + \
                                 self.output_activs.mm(self.output_to_output) # batch_size, n_outputs
    ...
    def draw_net(self, view=False, filename=None, node_names=None, node_colors=None, fmt='svg'):
    ...
    def create(genome, config, device='cpu'):
        ...
        for node_id, node in genome.nodes.items():
            ...
            assert node.aggregation == "sum", "this class only implements sum aggregation, please use RecurrentNetworkAgg for other types of aggregations"
        ...
        return RecurrentNetwork(input_neuron_ids, hidden_neuron_ids, output_neuron_ids, biases, responses, activations, connections, device=device)
    ...
    def update_genome(self, genome):
        for k, v in genome.nodes.items():
            if k in self.output_key_to_idx.keys():
                ...
                v.activation = '_'.join(
                    self.output_activations[self.output_key_to_idx[k]].__name__.split('_')[:-1])
            else:
                v.bias = self.hidden_biases[self.hidden_key_to_idx[k]].data
                v.responses = self.hidden_responses[self.hidden_key_to_idx[k]].data
                v.activation = '_'.join(
                    self.hidden_activations[self.hidden_key_to_idx[k]].__name__.split('_')[:-1])
                # v.aggregation = 'sum'
                ...
                if to_k in self.output_key_to_idx.keys():
                    if from_k in self.input_key_to_idx.keys():
                        v.weight = self.input_to_output[
                            self.input_key_to_idx[from_k], self.output_key_to_idx[to_k]].data
                    elif from_k in self.hidden_key_to_idx.keys():
                        v.weight = self.hidden_to_output[
                            self.hidden_key_to_idx[from_k], self.output_key_to_idx[to_k]].data
                    elif from_k in self.output_key_to_idx.keys():
                        v.weight = self.output_to_output[
                            self.output_key_to_idx[from_k], self.output_key_to_idx[to_k]].data
                else:
                    if from_k in self.input_key_to_idx.keys():
                        v.weight = self.input_to_hidden[
                            self.input_key_to_idx[from_k], self.hidden_key_to_idx[to_k]].data
                    elif from_k in self.hidden_key_to_idx.keys():
                        v.weight = self.hidden_to_hidden[
                            self.hidden_key_to_idx[from_k], self.hidden_key_to_idx[to_k]].data
                    elif from_k in self.output_key_to_idx.keys():
                        v.weight = self.output_to_hidden[
                            self.output_key_to_idx[from_k], self.hidden_key_to_idx[to_k]].data
...
class RecurrentNetworkAgg(nn.Module):
    ...
    def __init__(self, input_neuron_ids, hidden_neuron_ids, output_neuron_ids, biases, responses, activations, aggregations, connections, device='cpu'):
        """
        :param input_neuron_ids: list of ids of input neurons
        :param hidden_neuron_ids: list of ids of hidden neurons
        :param output_neuron_ids: list of ids of output neurons
        ...
        for k, v in connections.items():
            connections_with_str_keys[tuple_to_str_key(k)] = nn.Parameter(torch.tensor(v))
    ...
    def activate(self, inputs, n_passes=1):
        ...
        self.node_activs = torch.zeros((batch_size, self.n_outputs+self.n_hidden)).to(self.device)
        for _ in range(n_passes):
            outputs = self.forward(inputs)
    ...
    def draw_net(self, config=None, view=False, filename=None, node_names=None, node_colors=None, fmt='svg'):
        if node_names is None:
            node_names = {}
    ...
    def create(genome, config, device='cpu'):
        ...
        return RecurrentNetworkAgg(input_neuron_ids, hidden_neuron_ids, output_neuron_ids, biases, responses, activations, aggregations, connections, device=device)
    ...
    def update_genome(self, genome):
        for k, v in genome.nodes.items():
            ...
            v.activation = '_'.join(self.activation_functions[str(k)].__name__.split('_')[:-1])
            v.aggregation = '_'.join(self.aggregation_functions[str(k)].__name__.split('_')[:-1])
        for k, v in genome.connections.items():
            if v.enabled:
